...
export function DialogExportarCierres({ open, onOpenChange }: Props) {
  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ToastError({ message: "Error al generar el reporte de cierres." });
        return;
      }
      ...
      link.download = `reporte_cierres.${formato === "excel" ? "xlsx" : "pdf"}`;
      link.click();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md border-border">
        <DialogHeader>
          <DialogTitle>Exportar reporte de cierres</DialogTitle>
        </DialogHeader>
