...
import { IRecetaForm } from "@/lib/types";
  ...
  ingredientes: z
    .array(
      z.object({
        ...
        und_prod_rec: z.string(),
      }),
    )
    .min(1, "Debe agregar al menos un ingrediente"),
});

export function FormEditarReceta({
  receta,
  onSuccess,
  onClose, // 👈 nuevo prop
}: {
  ...
  const form = useForm<z.infer<typeof esquemaReceta>>({
    ...
    defaultValues: {
      nom_rec: receta.nom_rec,
      desc_rec: receta.desc_rec,
      prod_rec: receta.prod_rec?.id_prod?.toString() ?? "",
      pvp_rec: Number(receta.prod_rec?.prec_vent_prod) || 0,
      ingredientes: receta.ingredientes.map((ing: any) => ({
        prod_rec: ing.prod_rec.id_prod.toString(),
  ...
  useEffect(() => {
    const subscription = watch(async (value, { name }) => {
      if (name === "prod_rec") {
        ...
        if (seleccionado) {
          setValue("nom_rec", seleccionado.nombre);
        }
      }
  ...
  const onSubmit = async (data: IRecetaForm) => {
    ...
    try {
      ...
      const res = await fetch(SERVICIOS_RECETAS.actualizar(receta.id_rec), {
        method: "PUT",
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || "Error al actualizar receta");
      }
