...
export function DialogExportarGastos({ open, onOpenChange }: Props) {
  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ToastError({ message: "Error al generar el reporte de gastos." });
        return;
      }
      ...
      link.download = `reporte_gastos.${formato === "excel" ? "xlsx" : "pdf"}`;
      link.click();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Exportar reporte de gastos</DialogTitle>
        </DialogHeader>
